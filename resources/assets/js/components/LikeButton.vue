<template>
  <div class="btn-group btn-group-xs pull-left like-buttons" :style="signedIn ? 'width:83px' : 'width:89px' " role="group">
    <div class="emoji-icons-div" style="background:green;">
      <div class="box">
        <div class="Like">
          <a class="Like__link js-hover" style="visibility:hidden;">
            Like
            <kbd>hover me!!</kbd>
          </a>
          <div class="Emojis" v-if="visibleEmoticion">
            <div
              ...
              @mouseout="showLikeTypeUser = false"
            >
    ...
    <button
      class="btn btn-xs btn-default ml-a like-emoji-icon"
      @click="toggleLike(1, true)"
      @mouseenter="showEmoticion"
    >
      <span
        class="glyphicon glyphicon-thumbs-up like-icon"
        :class="likeClass"
      >&nbsp;{{ likesCount }}</span>
    </button>
    <button class="btn btn-xs btn-default ml-a" @click="toggleDislike">
      <span
        class="glyphicon glyphicon-thumbs-down like-icon"
        :class="dislikeClass"
      >&nbsp;{{ dislikesCount }}</span>
    </button>
    ...
    <div
      ...
      class="small-emoticion-icons"
      :class="{'small-emoticion-icons-left-margin':!isLiked}"
    >
      <div class="like-type-users" v-if="showLikeTypeUser">
        <span v-for="(user, index) in likeTypeUsers" :key="index">
          {{user}}
          <br />
        </span>
      ...
      <button
        class="emoji-like-counts"
        :class="smallEmojiClass(likeType)"
        @mouseenter="likeTypeUserHandle(likeType)"
        @mouseout="showLikeTypeUser = false"
        v-for=" (likeType, index) in allLikeTypes"
        :key="index"
      ></button>
...
export default {
  ...
  methods: {
    ...
    likeTypeUserHandle(type) {
      ...
      axios
        .get("/thread/" + this.thread.id + "/like-type-users/" + type)
        .then(res => {
