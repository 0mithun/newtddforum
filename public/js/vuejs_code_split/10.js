...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  computed: {
    pointCounts: function pointCounts() {
      ...
      return this.likeCount - this.dislikeCount; // return abbreviate((this.like_count - this.dislike_count), 1)
    }
  },
  created: function created() {
    ...
    eventBus.$on("threadDislikeDelete-" + this.thread.id, function (thread) {
      _this.getThreads();
    });
    eventBus.$on("threadDislikeAdd-" + this.thread.id, function (thread) {
      _this.getThreads();
    });
    eventBus.$on("threadLikeAdd-" + this.thread.id, function (thread) {
      _this.getThreads();
    });
    eventBus.$on("threadLikeDelete-" + this.thread.id, function (thread) {
      _this.getThreads();
    });
  },
  methods: {
...
var render = function() {
  ...
  return _c("div", { staticClass: "counts-item" }, [
    ...
    _vm._v("  " + _vm._s(_vm._f("strPlural")(_vm.pointCounts, "point")) + "\n")
  ])
}
var staticRenderFns = []
